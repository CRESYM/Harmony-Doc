...
The MMC's impedance characteristics are highly susceptible to time delays (computational time delay, sampling delay, modulation delay, etc.) in the system, particularly in the low (10�?00 Hz) and high (several kHz) frequency ranges @sakinci2022generalized. Therefore, these time delays must be considered during the modeling of the impedances. For simplicity, all time delays within the control architecture are consolidated into a single delay, based on the assumption that this aggregated delay can effectively represent the impact of the computational time delay on the impedance characteristics @sakinci2022generalized.

The time delay ($\tau_d$) can be expressed as a rational transfer function using the Pad{\'e} approximation stated in \eqref{eq:pade}, where $m$ and $n$ are the numerator and denominator approximation orders, respectively @sakinci2022generalized, @wang2016harmonic.
