...
The overhead transmission line model is constructed using the procedure from @martinez2017power and follows the previously developed code of the project authors @PowerImpedanceACDC2025. The model supports several conductor layout options, including flat, vertical, delta, offset, and concentric configurations. For greater flexibility, conductor positions can also be specified manually using absolute $(x,y)$ coordinates.

The simulator enables the creation of overhead lines using the conductor properties summarized in @tbl-ohl-conductors. The corresponding ground-wire properties are summarized in @tbl-ohl-groundwires.
...
Each conductor is positioned horizontally at $x_c$ relative to the tower center and vertically at $y_c$ above ground. Considering the sag at the midpoint between towers, denoted by $d_{sag}$, the modified vertical position used in the calculations is $\hat{y}_c = y_c - \frac{2}{3} \, d_{sag}$, as shown in @fig-tl. The conductor is formed using $n_{sb}$ sub-conductors grouped in the bundle, where all sub-conductors are grouped using a symmetrical equidistant pattern with the distance between the two nearest sub-conductors being $d_{bc}$, or a bundle spacing. Using the conductor position, the position of each sub-conductor can be estimated. Knowing the angle between two sub-conductors on the circle and its radius
...
The ABCD parameters of a transmission line can be defined as @630478, @772350
\begin{equation}
\begin{bmatrix}
\mathbf{A} & \mathbf{B} \\
\mathbf{C} & \mathbf{D}
...
From them Y parameters calculate as follows @630478, @772350:
\begin{eqnarray}
    Y = \begin{bmatrix}
