...
author: "Aleksandra LekiÄ‡, Haixiao Li"
...
1. **Frequency-Domain (Y-Parameter based)** â€?This model is used for steady-state AC analysis. For a single-phase capacitor with capacitance $C$, the admittance ($Y$) is given by:
    \begin{equation} 
    Y = j\omega C = sC 
    \end{equation}
...
2. **Time-Domain (MNA-based)** â€?This model is used for transient analysis or general MNA where the capacitor's behavior is integrated into the system equations. For a capacitor connected between two nodes, $n1$ and $n2$, with voltage difference $V_{n1} - V_{n2}$, and carrying current $I_C$, the fundamental relationship is:
